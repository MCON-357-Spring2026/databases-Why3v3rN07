...
def find_student_by_email(conn: sqlite3.Connection, email: str) -> Optional[sqlite3.Row]:
    ...
    TODO:
      ...
      - Use fetchone()
    """
    result = conn.execute("SELECT * FROM students WHERE email = ?;", (email,)).fetchone()
    return result
...
def rename_student(conn: sqlite3.Connection, student_id: int, new_name: str) -> int:
    ...
    """
    hits = conn.execute("UPDATE students SET name = ? WHERE id = ?;", (new_name, student_id))
    return hits.rowcount
...
def delete_student(conn: sqlite3.Connection, student_id: int) -> int:
    ...
    """
    hits = conn.execute("DELETE FROM students WHERE id = ?;", (student_id,))
    return hits.rowcount
