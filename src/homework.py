...
def add_student(conn: sqlite3.Connection, name: str, email: str) -> int:
    ...
    return conn.execute("INSERT INTO students (name, email) VALUES (?, ?);", (name, email)).lastrowid


def add_assignment(conn: sqlite3.Connection, title: str, max_points: int) -> int:
    """Insert into assignments and return new id."""
    return conn.execute("INSERT INTO assignments (title, max_points) VALUES (?, ?);", (title, max_points)).lastrowid
...
def record_grade(conn: sqlite3.Connection, student_id: int, assignment_id: int, score: int) -> int:
    ...
    return conn.execute("INSERT INTO grades (student_id, assignment_id, score) VALUES (?, ?, ?);", (student_id, assignment_id, score)).lastrowid
...
def student_grade_report(conn: sqlite3.Connection, student_id: int) -> list[sqlite3.Row]:
    ...
    """
    return conn.execute("""
    SELECT a.title AS assignment_title, g.score, a.max_points, ROUND(1.0 * g.score / a.max_points * 100, 1) AS percent
    FROM grades g
...
def leaderboard(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    ...
    """
    return conn.execute("""
    SELECT s.name as student_name, AVG(ROUND(1.0 * g.score / a.max_points * 100, 1)) AS avg_percent
    FROM grades g
    JOIN assignments a ON g.assignment_id = a.id
